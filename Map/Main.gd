...
func _process(delta):
	$Camera2D/LevelContainer/LevelNumberLabel.text = str(current_level)
	$Camera2D/EnemyContainer/EnemyCountNumberLabel.text = str(number_of_enemies_left)
...
func _on_enemy_died(points_worth_kill):
	var current_points = int($Camera2D/PointsContainer/pointCountNumberLabel.text)
	$Camera2D/PointsContainer/pointCountNumberLabel.text = str(points_worth_kill + current_points)
...
func _on_Player_current_health(current_health_percentage):
	$Camera2D/HealthContainer/HealthBar.value = current_health_percentage
...
func _on_Player_current_ammunition(ammunition):
	$Camera2D/AmmoContainer/AmmunitionCountLabel.text = str(ammunition)
